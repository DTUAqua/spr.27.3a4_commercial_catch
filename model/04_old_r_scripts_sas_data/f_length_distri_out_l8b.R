...
f_length_distri <- function(path_data, data_rca, data_rhh, data_rhl, data_rsl, data_nwg, data_ALK, path_graph, tab_levels, tab_time, tab_space, species, specie, years,
  ...
  # Group by sample
  eval(parse(text=paste0("l4 <- l1 %>% group_by(year,",time_first, ", rect, sampType, landCtry, vslFlgCtry, trpCode, staNum, subSampNo, length) %>% summarize_at(.vars = vars(vgt, number), .funs = sum)")))
