...
f_length_distri <- function(path_data, data_rca, data_rhh, data_rhl, data_rsl, data_nwg, data_ALK, path_graph, tab_levels, tab_time, tab_space, species, specie, years,
  ...
  rca$indWt[rca$year == 1983] <- rca$indWt[rca$year == 1983]*1000
  ...
  rhl <- left_join(rhl, select(rhh, -sampType, -recType, -X))
  rhl$day <- substr(rhl$date, 9, 10)
  rhl$day <- as.numeric(rhl$day)
  
  unique(is.na(rhl$day))
  unique(is.na(rhl$date))
  
  # Length - need to be changed
  rca$length <- rca$lenCls
  rhl$length <- rhl$lenCls
  rhl$lenCode <- "scm"
  if(type_length == "scm"){
    ...
    rhl$length[rhl$lenCode == 'scm'] <- (floor(rhl$lenCls[rhl$lenCode == 'scm']*2)/2)/10
    rhl$length[rhl$lenCode == 'mm'] <- 0.5*floor(rhl$lenCls[rhl$lenCode == 'mm']*0.2) 
  }
  ...
  grp_rca <- rca %>% group_by (sampType, landCtry, vslFlgCtry, year, month, quarter, day, proj, trpCode, staNum, rect, length) %>%
    summarise(number = n(), vgt = sum(indWt)/1000, .groups = 'drop' )
  ...
  grp_rhl <- rhl %>% group_by (sampType, landCtry, vslFlgCtry, year, proj, trpCode, staNum) %>%
    summarise(subSampNo = sum(lenNum, na.rm = T), .groups = 'drop' )
  
  rhl_1 <- merge(rhl, grp_rhl)
  
  unique(is.na(rhl_1$number))
  
  hlca <- left_join(rhl_1, select(grp_rca, -sampType))
  grp_rca <- hlca
  ...
  eval(parse(text=paste0("nb_samples <- l2 %>% group_by(year,", time_first, ", rect) %>% summarise(n_samples = n(), .groups = 'drop' )")))
